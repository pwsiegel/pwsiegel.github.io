...
$$\eta\_{C_2} \circ F(f) = G(f) \circ \eta\_{C_1}$$
...
To complete the exercise, we just also show that every natural transformation $\eta \colon F_1 \to F_2$ determines a graph homomorphism $G_1 \to G_2$.
