...
$$\text{PSh}(\mathcal{C}) = \text{Fun}(\mathcal{C}^\opp, \Set)$$
...
The upshot of the two exercises on graphs in this post is that the category of graphs equipped with graph homomorphisms is equivalent to the category of presheaves over the seemingly innocuous category $X^\opp$ with two objects and two morphisms from one object to the other.
