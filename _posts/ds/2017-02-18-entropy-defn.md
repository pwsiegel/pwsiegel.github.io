...
Here's an example.  Consider a weighted coin flip, with $\Omega = \br{H, T}$, $\Sigma = \br{\emptyset, \br{H}, \br{T}, \Omega}, and probabilities $\P(\br{H}) = \frac{1}{4}$, $\P(\lbrace T \rbrace) = \frac{3}{4}$.  (Of course, $\P(\emptyset) = 0$ and $\P(\Omega) = 1$.)  There are only a few independent events in this probability space: the events $\lbrace H \rbrace$ and $\lbrace T \rbrace$ are each independent of $\emptyset$ and $\Omega$, but that's it!  These events are enough to force $S(\emptyset) = \infty$ and $S(\Omega) = 0$ (see below), but the only other constraint on $S$ is that $S(\lbrace H \rbrace) > $S(\lbrace T \rbrace)$.  By contrast, if the formula $S(E) = -\log \P(E)$ were forced upon us then we would have the constraint:

$$S(\lbrace H \rbrace) - S(\lbrace T \rbrace) = -\log 4 + \log \frac{3}{4} = \log 3$$
